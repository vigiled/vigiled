...
function setMode(n){
  send('MODE:'+n);
  document.querySelectorAll('.mb').forEach(b=>b.classList.toggle('act',parseInt(b.dataset.m)===n));
}
...
void handleNotFound() {
  // Portail captif iOS : redirige toute URL inconnue vers la page
...
void gererBoutonBright() {
  ...
  if (btn && etatBtnBright==0) { etatBtnBright=1; tempsAppuiBright=millis(); dernierAjustBri=millis(); }
...
void setup() {
  ...
  Serial.println("AP pret : " + String(AP_SSID));
  Serial.println("IP      : " + WiFi.softAPIP().toString());

  server.on("/",        handleRoot);
  server.on("/cmd",     handleCmd);
  server.onNotFound(handleNotFound);  // portail captif iOS
  server.begin();
  Serial.println("HTTP OK — ouvrir http://192.168.4.1 dans Safari");
}
